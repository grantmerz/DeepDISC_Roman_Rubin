...
def combine_metadata(annotations_dir, all_metadata_path):
    """Loads and combines all tile.json files from a directory into a single file"""
...
def clean_metadata_of_bad_roman(all_metadata_path, clean_metadata_path, integrity_report_path):
    """
    Filters out entries corresponding to corrupt Roman cutouts based on the integrity report
    created by `check_cutout_integrity.py`.
    
    Args:
        all_metadata_path: Path to the combined metadata file
        clean_metadata_path: Path to save the cleaned metadata
        integrity_report_path: Path to the Roman cutout integrity report JSON
    """
    print(f"Cleaning metadata based on integrity report: {integrity_report_path}")
    if not os.path.exists(integrity_report_path):
        print(f"Warning: Integrity report not found at {integrity_report_path}")
        print("Skipping cleaning step. Using all metadata as-is.")
    ...
    # set of corrupt Roman cutout filenames for fast lookup
    corrupt_filenames = set(integrity_report.get('corrupt_cutouts', {}).keys())
    print(f"Found {len(corrupt_filenames)} corrupt Roman cutouts in the integrity report")

    with open(all_metadata_path, 'r') as f:
        all_metadata = json.load(f)
    print(f"Total entries before cleaning: {len(all_metadata)}")
    # now let's just filter out entries with corrupt Roman cutouts
    # LSST cutout path is in entry['file_name'] so we need to check it against corrupt_filenames
    # e.g., /u/yse2/lsst_data/truth/50.99_-38.4/c208_50.99_-38.4.npy
    valid_entries = [
        entry for entry in all_metadata
        if Path(entry['file_name']).name not in corrupt_filenames # e.g., c208_50.99_-38.4.npy
    ]
    # valid_entries = [entry for entry in all_metadata if entry['height'] > 0 and entry['width'] > 0]
    removed_count = len(all_metadata) - len(valid_entries)
    print(f"Removed {removed_count} entries corresponding to corrupt Roman cutouts")
    print(f"Valid entries after cleaning: {len(valid_entries)}")
    
    with open(clean_metadata_path, 'w') as f:
        json.dump(valid_entries, f, indent=4)
    
    print(f"Saved cleaned metadata to {clean_metadata_path}")
    return clean_metadata_path

def split_and_save_data(metadata_path, output_dir, train_ratio=0.7, val_ratio=0.1):
...
def generate_plots(output_dir, snr_lvl, band):
    ...
    # assuming output_dir is annotations_dir
    train_path = os.path.join(output_dir, 'train.json')
    val_path = os.path.join(output_dir, 'val.json')
    test_path = os.path.join(output_dir, 'test.json')
...
def main():
    ...
    parser.add_argument('--root_dir', type=str, default='/work/hdd/bfhm/yse2/lsst_data/',
    ...
    parser.add_argument('--integrity_report', type=str, 
                        default='/u/yse2/roman_cutout_integrity_report.json',
                        help='Path to the Roman cutout integrity report JSON file.')
    
    
    args = parser.parse_args()

    annotations_dir = os.path.join(args.root_dir, f'annotations_lvl{args.snr_lvl}/')
    all_metadata_path = os.path.join(annotations_dir, 'all_metadata.json')
    clean_metadata_path = os.path.join(annotations_dir, 'all_metadata_clean.json')
    
    if not os.path.exists(all_metadata_path):
        print("STEP 1: Combining metadata files")
        combine_metadata(annotations_dir, all_metadata_path)
    else:
        print(f"All metadata file already exists: {all_metadata_path}")

    # clean metadata based on integrity report
    if not os.path.exists(clean_metadata_path):
        print("STEP 2: Cleaning metadata based on Roman cutout integrity")
        clean_metadata_of_bad_roman(all_metadata_path, clean_metadata_path, args.integrity_report)
    else:
        print(f"Clean metadata file already exists: {clean_metadata_path}")
