...
RA_DEC_PATTERN = re.compile(r'dc2_(det|seg|index)_(\d+\.\d+)_(\-\d+\.\d+)')

def extract_ra_dec(filename):
    matches = RA_DEC_PATTERN.search(filename)
    if matches:
        return matches.group(2), matches.group(3)
    return None

def create_multiband_coadd(ra_dec):
    target_file = f"/home/shared/hsc/roman_lsst/roman_data/truth/{ra_dec[0]}_{ra_dec[1]}/full_{ra_dec[0]}_{ra_dec[1]}.npy"
...
#     h158_img = fits.open(f'roman_data/original_fits/{ra_dec[0]}_{ra_dec[1]}/H158_{ra_dec[0]}_{ra_dec[1]}.fits')
    h158_img = fits.open(f'/home/shared/hsc/roman_lsst/original_fits/{ra_dec[0]}_{ra_dec[1]}/dc2_H158_{ra_dec[0]}_{ra_dec[1]}.fits')
    w = WCS(h158_img[1].header)
    ...
    y106_img = fits.open(f'/home/shared/hsc/roman_lsst/original_fits/{ra_dec[0]}_{ra_dec[1]}/dc2_Y106_{ra_dec[0]}_{ra_dec[1]}.fits')
    j129_img = fits.open(f'/home/shared/hsc/roman_lsst/original_fits/{ra_dec[0]}_{ra_dec[1]}/dc2_J129_{ra_dec[0]}_{ra_dec[1]}.fits')
#     full_img_data = np.stack((f184_img[1].data, h158_img[1].data, y106_img[1].data, j129_img[1].data))
    full_img_data = np.stack((y106_img[1].data, j129_img[1].data, h158_img[1].data))
...
def create_cutouts(seg_file, full_img_data, ra_dec, w):
    seg = fits.open(seg_file, memmap=True)
    cutout_size = 512
    overlap_pixels = 500
    ...
    for i in range(ny_cutouts):
        for j in range(nx_cutouts):
           # cutout center pos
            ...
            full_cutout_path = f'/home/shared/hsc/roman_lsst/roman_data/truth/{ra_dec[0]}_{ra_dec[1]}/c{counter}_{ra_dec[0]}_{ra_dec[1]}.npy'
            if not os.path.exists(full_cutout_path):
                raw_cutout_y106 = Cutout2D(full_img_data[0], position=(x_center, y_center), 
                                         size=cutout_size, wcs=w, mode='partial', fill_value=0)
                raw_cutout_j129 = Cutout2D(full_img_data[1], position=(x_center, y_center), 
                                         size=cutout_size, wcs=w, mode='partial', fill_value=0)
                raw_cutout_h158 = Cutout2D(full_img_data[2], position=(x_center, y_center), 
                                         size=cutout_size, wcs=w, mode='partial', fill_value=0)
                full_raw_cutout = np.stack((raw_cutout_y106.data, raw_cutout_j129.data, raw_cutout_h158.data))
...
def get_cutout_truth_cat(seg_cutouts, truth_df, ra_dec, verbose=False):
    ...
    filtering the truth catalog to only objects within those RA/Dec ranges. For convenience, we also convert the filtered objects' coordinates 
    ...
    for imgid, seg_cutout in enumerate(seg_cutouts):
        truth_cutout_path = f'/home/shared/hsc/roman_lsst/roman_data/truth/{ra_dec[0]}_{ra_dec[1]}/truth_c{imgid}_{ra_dec[0]}_{ra_dec[1]}.json'
...
def get_cutout_det_cat(seg_cutouts, det_df, ra_dec, verbose=False):
    ...
    For convenience, we also convert the det objects' coords to cutout pixel coords.
    """
    ...
    for imgid, seg_cutout in enumerate(seg_cutouts):
        det_cutout_path = f'/home/shared/hsc/roman_lsst/roman_data/truth/{ra_dec[0]}_{ra_dec[1]}/det_c{imgid}_{ra_dec[0]}_{ra_dec[1]}.json'
...
def process_single_cutout(args):
    ...
    metadata["file_name"] = f'roman_data/truth/{ra_dec[0]}_{ra_dec[1]}/c{cutout_id}_{ra_dec[0]}_{ra_dec[1]}.npy'
...
def create_cutout_metadata(seg_cutouts, det_dfs, truth_dfs, ra_dec, n_workers=96):
    """Using 16 workers"""
        ...
        for future in tqdm(as_completed(future_to_cutout), 
            ...
            try:
                ...
                if result['success']:
                    ...
                    matched_file = f'/home/shared/hsc/roman_lsst/roman_data/truth/{ra_dec[0]}_{ra_dec[1]}/matched_c{cutout_id}_{ra_dec[0]}_{ra_dec[1]}.json'
    ...
    os.makedirs('/home/shared/hsc/roman_lsst/roman_data/annotations', exist_ok=True)
    metadata_filename = f"/home/shared/hsc/roman_lsst/roman_data/annotations/{ra_dec[0]}_{ra_dec[1]}.json"
    convert_to_json(all_metadata, metadata_filename, allow_cached=False)
...
def main(tile_list_file):
    shared_dir = '/home/shared/hsc/roman_lsst/'
    detection_dir = f'{shared_dir}roman_data/detection_fits'
    segmentation_dir = f'{shared_dir}roman_data/segmentation_fits'
    truth_dir = f'{shared_dir}roman_data/truth_fits'
    
    with open(tile_list_file, 'r') as f:
        tiles = [line.strip() for line in f if line.strip()]
        
#     tiles = ['50.94_-39.9', '50.94_-41.4', '50.95_-39.0']
    det_files = [f'{detection_dir}/dc2_det_{tile}.fits.gz' for tile in tiles]

    total_start_time = time.time()
    print(f"Processing {len(det_files)} tiles...")
    for i, det_file in enumerate(tqdm(det_files, desc=f"Processing Tiles", unit="tile")):
    #     if i == 0 or i == 1:
    #         continue
        tile_start_time = time.time()
        ra_dec = extract_ra_dec(det_file)
        os.makedirs(f'{shared_dir}roman_data/truth/{ra_dec[0]}_{ra_dec[1]}', exist_ok=True)
        print(f"\n{'='*50}")
        ...
        print(f"Time for Multiband coadd: {coadd_time:.2f}s")
        ...
        del full_img_data
        gc.collect()
        print(f"Time for Create cutouts: {cutouts_time:.2f}s")
        ...
        del seg_cutouts, truth_df, det_df, cutout_truth_dfs, cutout_det_dfs
        gc.collect()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process Roman data tiles.")
    parser.add_argument("--tile_list", required=True, help="Path to a text file with a list of tiles to process.")
