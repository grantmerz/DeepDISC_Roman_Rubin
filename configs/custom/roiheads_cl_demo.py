...
def _supervised_contrastive_loss(logits, pos_mask):
    ...
    so every anchor always has at least one positive (itself z_q_i / z_q_i). 
    This means p is never all-zero and no anchor is ever skipped
...
class ContrastiveCascadeROIHeads(CascadeROIHeads):
    ...
    Contrastive flow (ROI-level InfoNCE, only during training with paired Roman data):
        ...
        6. Compute InfoNCE loss: treat pairs with the same gt_objid as
    ...
    def forward_contrastive(self, features_q, instances, targets):
        """
        Compute the ROI-level InfoNCE (CLIP-style) contrastive loss
        using SupCon - positive pairs are determined by matching proposals to the ground truth boxes
        ...
        if self.training:
            ...
            # temperature: logits[i, j] = sim(z_q_i, z_q_j) / T
            # Shape: (N, N); diagonal entries are pos pairs
            scale = self.logit_scale.exp()
            logits = (z_q @ z_q.T) * scale # (N', N') — q anchors, q keys
            # Positive mask is symmetric (same objid relation in both directions)
            loss = _supervised_contrastive_loss(logits, pos_mask)
            return {
    ...
    def forward(self, images, features_q, proposals, targets=None):
        ...
        During training:
            ...
            3. _forward_mask: runs the mask head on foreground proposals,
               returning mask loss
            4. forward_contrastive: computes SupCon contrastive loss on proposals
        ...
        Returns:
            training: (proposals, losses_dict)
                proposals are the labeled/sampled proposals
        ...
        if self.training:
            ...
            losses.update(self.forward_contrastive(features_q,proposals,targets))
