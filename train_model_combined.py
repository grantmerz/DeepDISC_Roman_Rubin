...
# the 
def combined_key_mapper(dataset_dict):
    '''
    args
        dataset_dict: [dict]
    ...
    suffix = dataset_dict['file_name'].split('./lsst_data/truth-ups/')[1][4:]
    return suffix
...
def main(args, freeze):
    ...
    trainfile = "/home/g4merz/DeepDISC_Roman_Rubin/data/annotations_lvl2_ups_train_clean.json"
    evalfile = "/home/g4merz/DeepDISC_Roman_Rubin/data/annotations_lvl2_ups_val_clean.json"
    ...
    with open(trainfile, "r") as read_file:
        ddicts = json.load(read_file)
        l = len(ddicts)

    batch_size = cfg.dataloader.train.total_batch_size
    epoch = int(l / batch_size)
    ...
    mapper = cfg.dataloader.train.mapper(
            cfg.dataloader.imagereader, combined_key_mapper, cfg.dataloader.augs
        ).map_data
    ...
    if freeze:
        ...
        saveHook = return_savehook(run_name,save_period=epoch*5)
